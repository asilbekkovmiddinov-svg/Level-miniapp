(function (root, factory) {
    ...
    class PromotionsAdminError extends Error {
        constructor(message, status = 0) { super(message); this.name = "PromotionsAdminError"; this.status = status; }
    }
    class PromotionsAdminApi {
        ...
        async request(path, { method = "GET", body } = {}) {
            ...
            const response = await this.fetchImpl(`${this.baseUrl}${path}`, { method, headers: { "X-Telegram-Init-Data": initData, ...(body ? { "Content-Type": "application/json" } : {}) }, ...(body ? { body: JSON.stringify(body) } : {}) });
            let payload = null; try { payload = await response.json(); } catch (_error) {}
            if (!response.ok) throw new PromotionsAdminError(payload?.detail || (response.status === 401 ? "Admin login required." : response.status === 403 ? "Admin permission required." : "Admin so‘rovini bajarib bo‘lmadi."), response.status);
        ...
        async exportAnalytics(period = "7D") { const initData = this.initDataProvider(); if (!initData) throw new PromotionsAdminError("Admin login required.", 401); const response = await this.fetchImpl(`${this.baseUrl}/admin/promotions/analytics/export?period=${encodeURIComponent(period)}`, { headers: { "X-Telegram-Init-Data": initData } }); if (!response.ok) throw new PromotionsAdminError("CSV export failed.", response.status); return { blob: await response.blob(), filename: `promotion-analytics-${period.toLowerCase()}.csv` }; }
        ...
        uploadBanner(id, blob, onProgress = () => {}, xhrFactory = () => new XMLHttpRequest()) {
            const initData = this.initDataProvider(); if (!initData) return Promise.reject(new PromotionsAdminError("Admin login required.", 401));
            return new Promise((resolve, reject) => { const xhr = xhrFactory(); xhr.open("POST", `${this.baseUrl}/admin/promotions/${Number(id)}/banner`); xhr.setRequestHeader("X-Telegram-Init-Data", initData); xhr.upload.onprogress = (event) => { if (event.lengthComputable) onProgress(Math.round((event.loaded / event.total) * 100)); }; xhr.onerror = () => reject(new PromotionsAdminError("Banner upload network error.")); xhr.onload = () => { let payload = null; try { payload = JSON.parse(xhr.responseText || "null"); } catch (_error) {} if (xhr.status < 200 || xhr.status >= 300) return reject(new PromotionsAdminError(payload?.detail || "Banner upload failed.", xhr.status)); if (!payload) return reject(new PromotionsAdminError("Serverdan noto‘g‘ri javob olindi.", xhr.status)); onProgress(100); resolve(payload); }; const form = new FormData(); const extension = blob.type === "image/png" ? "png" : blob.type === "image/jpeg" ? "jpg" : "webp"; form.append("file", blob, `promotion-banner.${extension}`); xhr.send(form); });
        }
    }
